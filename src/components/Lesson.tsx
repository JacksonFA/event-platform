...
export function Lesson(props: LessonProps) {
    ...
    return (
        <Link to={isLessonAvailable ? `/event/lesson/${props.slug}` : '#'} className={`group ${!isLessonAvailable ? 'cursor-not-allowed' : ''}`}>
            <span className="text-gray-300">
                {availableDateFormatted}
            </span>

            <div className={`rounded border border-gray-500 p-4 mt-2 ${isLessonAvailable ? 'group-hover:border-green-500' : ''} ${isActiveLesson ? 'bg-green-500' : ''}`}>
